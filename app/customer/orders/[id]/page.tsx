...
export default function CustomerOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-8 text-center text-slate-100 space-y-4">
        <p className="font-bold text-lg">Pesanan tidak ditemukan.</p>
        <Link href="/customer/history" className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold">
          Kembali ke Riwayat
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4 md:p-8 font-sans text-slate-100">
      <div className="max-w-xl mx-auto space-y-6">
        <Link href="/customer/history" className="flex items-center gap-2 text-blue-400 font-bold hover:underline mb-2">
        ...
        <div className="bg-slate-800/90 backdrop-blur-xl p-6 rounded-3xl border border-slate-700/60 shadow-xl space-y-6">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[10px] font-black uppercase tracking-wider px-3 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full">
                {order.service || 'GASKE'}
              </span>
              <h2 className="text-xl font-black text-white mt-2">Detail Pesanan</h2>
              <p className="text-xs font-medium text-slate-400 mt-0.5">Resi: {order.order_number || order.id.slice(0, 8)}</p>
            </div>
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${order.status === 'COMPLETED' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'bg-amber-500/15 text-amber-400'}`}>
              {order.status}
            </span>
          </div>

          {/* Info Driver */}
          <div className="flex items-center gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-700/50">
            ...
            <div className="min-w-0">
              <h4 className="font-bold text-white truncate">{order.driver_name || 'Mencari Driver...'}</h4>
          ...
          <div className="space-y-3 text-xs">
            <div className="flex items-start gap-3 bg-slate-900/60 p-3.5 rounded-2xl border border-slate-700/50">
              <span className="w-5 h-5 rounded-full bg-blue-500 text-white font-bold flex items-center justify-center shrink-0 text-[10px] mt-0.5">A</span>
              <div className="min-w-0">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Titik Jemput / Toko</p>
                <p className="font-bold text-white mt-0.5 break-words">{order.pickup_address}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 bg-slate-900/60 p-3.5 rounded-2xl border border-slate-700/50">
              ...
              <div className="min-w-0">
                ...
                <p className="font-bold text-white mt-0.5 break-words">{order.destination_address}</p>
              </div>
            </div>
          </div>

          {/* Rincian Biaya & Pembayaran */}
          <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-700/50 space-y-2 text-xs">
            <div className="flex justify-between text-slate-400">
              <span>Jarak Tempuh</span>
              <span className="font-bold text-slate-200">{order.distance_km || 0} KM</span>
            </div>
            <div className="flex justify-between text-slate-400">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-slate-700/60 text-sm">
              <span className="font-black text-slate-300">Total Biaya</span>
          ...
          {order.status === 'COMPLETED' && (
            <div className="bg-blue-950/30 p-4 rounded-2xl border border-blue-500/20 space-y-3">
              <h3 className="text-xs font-black text-white uppercase tracking-wider">Ulasan & Rating Driver</h3>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star 
                        key={star} 
                        className={`w-4 h-4 ${star <= order.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-600'}`} 
                      />
                    ))}
                    <span className="text-xs font-bold text-slate-300 ml-2">({order.rating}/5)</span>
                  </div>
                  {order.review && <p className="text-xs text-slate-300 italic">"{order.review}"</p>}
                  <p className="text-[10px] text-blue-400 font-medium">Ulasan telah dikirimkan.</p>
                </div>
              ) : (
                <form onSubmit={handleRatingSubmit} className="space-y-3">
                  <div className="flex items-center gap-1.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star 
                          className={`w-5 h-5 transition ${star <= ratingInput ? 'text-amber-400 fill-amber-400 scale-110' : 'text-slate-600'}`} 
                        />
                      </button>
                    ))}
                    <span className="text-xs font-bold text-slate-300 ml-2">{ratingInput} Bintang</span>
                  </div>
                  ...
                  <textarea
                    ...
                    className="w-full text-xs p-3 rounded-xl border border-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-900 text-white"
                    rows={2}
                  />
