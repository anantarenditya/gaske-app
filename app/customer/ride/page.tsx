...
import { ArrowLeft, Loader2, Bike, Search, LocateFixed, CreditCard, QrCode, X, MapPin } from 'lucide-react';
...
export default function GaskeRidePage() {
  ...
  const executeCheckout = async (method: 'Tunai (Cash)' | 'QRIS') => {
    ...
    const res = await createOrderAction({
      ...
      destinationAddress: destinationAddress || 'Lokasi Tujuan',
      distanceKm,
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 w-full max-w-sm rounded-[2.5rem] p-6 shadow-2xl space-y-5 text-center relative text-slate-100">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-400 font-bold mt-1">Total Tarif: {formatRupiah(fare)}</p>
            </div>
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-400 font-semibold mt-1">Antar Jemput Cepat & Aman</p>
            </div>
          </div>
          <button type="button" onClick={handleCurrentGPS} className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-3 py-2 rounded-xl flex items-center gap-1.5">
      ...
      <main className="max-w-md mx-auto px-4 pt-5 space-y-4">
        <form onSubmit={(e) => { e.preventDefault(); searchPlacesReal(searchQuery); }} className="relative">
          <input
            ...
            className="w-full p-3.5 pl-10 pr-20 bg-slate-800/90 border border-slate-700 rounded-2xl text-xs font-bold text-white shadow-sm focus:ring-2 focus:ring-emerald-500"
          />
        ...
        {loadingSearch && (
          <div className="flex items-center gap-2 py-2 justify-center text-xs text-slate-400">
            <Loader2 className="w-4 h-4 animate-spin text-emerald-400" /> Mencari lokasi...
          </div>
        )}
            ...
            <div className="space-y-1.5">
              {places.map((place) => (
                <button
                  ...
                  className="w-full p-3 rounded-2xl border border-slate-700 text-left bg-slate-900/60 hover:bg-emerald-500/10 hover:border-emerald-500/50 transition flex flex-col gap-0.5 shadow-sm"
                >
        ...
        <div className="bg-slate-800/90 backdrop-blur-xl p-4 rounded-3xl border border-slate-700/60 shadow-xl space-y-3">
          ...
          <div className="flex justify-between items-center pt-1">
            <span className="text-xs font-bold text-slate-300">Jarak: <strong className="text-emerald-400">{distanceKm.toFixed(1).replace('.', ',')} KM</strong></span>
            <button
              type="button"
        ...
        <div className="bg-slate-800/90 backdrop-blur-xl p-4 rounded-3xl border border-slate-700/60 shadow-xl space-y-3">
          <div className="space-y-1 bg-slate-900/60 p-3 rounded-2xl border border-slate-700/50">
            <label className="text-[10px] font-extrabold text-emerald-400 uppercase tracking-wider flex items-center gap-1">
              <MapPin className="w-3.5 h-3.5" /> Lokasi Jemput (A):
            </label>
          ...
          <div className="space-y-1 bg-slate-900/60 p-3 rounded-2xl border border-slate-700/50">
            <label className="text-[10px] font-extrabold text-rose-400 uppercase tracking-wider flex items-center gap-1">
              <MapPin className="w-3.5 h-3.5" /> Lokasi Tujuan (B):
            </label>
            ...
            {[
              ...
              <button
                ...
                className={`p-3 rounded-2xl border text-xs font-black transition flex items-center justify-center gap-2 ${
                  paymentMethod === m.label ? 'bg-emerald-600 text-white border-emerald-500 shadow-md' : 'bg-slate-900/60 text-slate-300 border-slate-700'
                }`}
              >
                {m.icon} {m.label}
              </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 z-30 shadow-2xl">
        <div className="max-w-md mx-auto space-y-2.5">
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-400 font-medium">Tarif Perjalanan:</span>
            <span className="font-black text-white text-base">{formatRupiah(fare)}</span>
          </div>
          <button onClick={handleCheckout} disabled={loadingCheckout} className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold rounded-2xl shadow-lg transition flex items-center justify-center gap-2 text-xs">
            {loadingCheckout ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Bike className="w-4 h-4" /> Pesan Gaske Ride ({paymentMethod})</>}
          </button>
        </div>
      </div>
    </div>
