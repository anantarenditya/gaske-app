...
import { ArrowLeft, Package, Loader2, User, Phone, FileText, CreditCard, Search, MapPin, LocateFixed, QrCode, X } from 'lucide-react';
import dynamic from 'next/dynamic';

const DynamicMap = dynamic(() => import('../Map'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full bg-slate-800 animate-pulse flex items-center justify-center text-xs text-slate-400">
      Memuat Peta...
    </div>
  ),
});
...
export default function SendPage() {
  ...
  const [pickup, setPickup] = useState('Mendeteksi lokasi Anda...');
  const [destination, setDestination] = useState('Pakuwon Mall Surabaya');
  ...
  const handleManualGPS = () => {
    setPickup('Sedang melacak lokasi...');
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          const lat = pos.coords.latitude;
  ...
  const executeOrder = async (method: 'Tunai (Cash)' | 'QRIS') => {
    ...
    const { error } = await supabase.from('orders').insert({
      ...
      destination_address: `${destination} | ${detailPenerima}`,
    ...
    else { setShowQrisModal(false); router.push('/customer'); }
  };

  const handleOrderSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pickup || !destination || !receiverName) { alert('Lengkapi alamat dan nama penerima!'); return; }
    ...
    <div className="min-h-screen bg-slate-900 pb-28 font-sans text-slate-100 relative">
      ...
      <header className="bg-slate-800/90 backdrop-blur-xl border-b border-slate-700/60 sticky top-0 z-30 px-5 py-4 shadow-xl">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className="text-[10px] font-bold text-rose-400 bg-rose-500/10 border border-rose-500/30 px-3 py-2 rounded-xl flex items-center gap-1.5 transition hover:bg-rose-500/20"
          >
      ...
      <main className="max-w-md mx-auto px-4 pt-5 space-y-4">
          ...
          <div className="relative z-20">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-xs font-semibold text-white focus:outline-none focus:ring-2 focus:ring-rose-500 shadow-sm"
              />
              {isSearching && <Loader2 className="w-4 h-4 animate-spin text-rose-400 absolute right-3" />}
            </div>
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl z-50 overflow-hidden divide-y divide-slate-700 max-h-52 overflow-y-auto">
                {searchResults.map((item, index) => (
                  <button
                    ...
                    className="w-full text-left p-3 hover:bg-rose-500/10 transition flex items-start gap-2.5 text-xs text-slate-200 font-medium"
                  >
        ...
        <form onSubmit={handleOrderSubmit} className="bg-slate-800/90 backdrop-blur-xl p-6 rounded-3xl border border-slate-700/60 shadow-xl space-y-4">
          <div className="space-y-3 text-xs">
            <div className="flex items-start gap-2.5 bg-slate-900/60 p-3 rounded-2xl border border-slate-700/50">
              ...
              <div className="truncate">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Lokasi Ambil (GPS Anda)</p>
                <p className="font-bold text-white truncate">{pickup}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5 bg-slate-900/60 p-3 rounded-2xl border border-slate-700/50">
              ...
              <div className="truncate">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Lokasi Tujuan</p>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-rose-400" /> Detail Barang</label>
            <input type="text" value={itemDetail} onChange={(e) => setItemDetail(e.target.value)} placeholder="Contoh: Dokumen / Kotak Makanan" required
              className="w-full p-3.5 bg-slate-900/90 rounded-2xl text-xs text-white border border-slate-700 font-bold focus:outline-none focus:ring-2 focus:ring-rose-500" />
          </div>
          ...
          <div className="pt-3 border-t border-slate-700/60 flex justify-between items-center">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase">Tarif SEND ({distanceKm} km)</p>
              <p className="text-lg font-black text-white">{formatRupiah(price)}</p>
            </div>
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-3.5 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white font-black rounded-2xl text-xs shadow-lg flex items-center gap-2 transition"
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Kirim Paket'}
            </button>
          </div>
        </form>
      </main>
